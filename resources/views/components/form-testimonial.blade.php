...
<div :class="$store.formTestimonial.show === false ? 'hidden' : 'flex'"
    ...
    <div class="w-auto h-2/4 lg:w-2/6 px-3 py-4 bg-gray-800 border border-gray-600 rounded-md flex flex-col overflow-y-auto"
        ...
        <form method="POST" :action="isEdit ? '{{ $routeStore }}' : '{{ $routeEdit }}'" class="flex flex-col"
            ...
            <div class=" flex flex-col justify-start mb-2">
                ...
                <x-text-input id="name" name="name" class=" outline-none w-full p-2 mt-2 bg-gray-700 text-white/80 border-gray-600"
                    placeholder="Enter the customer name" autofocus required></x-text-input>
            ...
            <div class=" flex flex-col justify-start mb-2">
                ...
                <x-text-input id="position" name="position" class=" outline-none w-full p-2 mt-2 bg-gray-700 text-white/80 border-gray-600"
                    placeholder="Enter the customer's job position, e.g., Karyawan" required></x-text-input>
                @error('position')
                    <p class="text-sm text-red-400 mt-1">{{ $message }}</p>
                @enderror
            </div>
            <div class=" flex flex-col justify-start mb-2">
                ...
                <x-textarea id="comment" name="comment" placeholder="Enter the customer feedback"
                    class=" mt-2 bg-gray-700 text-white/80 border-gray-600" required></x-textarea>
                @error('comment')
                    <p class="text-sm text-red-400 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class=" flex flex-col justify-start mb-2">
                ...
                <x-text-input id="rating" name="rating" type="number" max="5" min="0"
                    class=" outline-none w-full p-2 mt-2 bg-gray-700 text-white/80 border-gray-600" required></x-text-input>
            ...
            <div class=" flex flex-col justify-start mb-4">
                ...
                <x-select-input id="isShow" name="isShow" class=" mt-2 text-white/80 bg-gray-700 border-gray-600" required>
